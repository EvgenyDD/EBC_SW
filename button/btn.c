...
{
	if(cmd == 0)		OCR0A = var;
	else if(cmd == 1)	OCR1A = var;
	else if(cmd == 2)	OCR0B = var;
	else if(cmd == 3)	OCR1B = var;
}
...
int main(void)
{
	...
	UBRRH = (unsigned char)(8>>8); // задаем скорость
    UBRRL = (unsigned char)8;     // в бодах, т.е. бит\с
    UCSRB = (1<<TXEN)|(1<<RXEN)|(1<<RXCIE);    // разрешаем передачу и прием
